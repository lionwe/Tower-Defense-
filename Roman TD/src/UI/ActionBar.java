...
public class ActionBar extends Bar {
	...
	private void initButtons() {
		...
		for (int i = 0; i < towerButtons.length; i++) {
			...
			sellTower = new MyButton("Продати", 420, 702, 80, 25);
			upgradeTower = new MyButton("Покращити", 545, 702, 80, 25);
	...
	public void draw(Graphics g) {
		...
		if(playing.isGamePaused()) {
			...
			g.drawString("Гра на паузі", 110, 790);
		}
	...
	private void drawTowerCost(Graphics g) {
		...
		g.drawString("Ціна: " + getTowerCostCost() + " з", 285, 695);

		// show this if player lacks for the selected tower

		if (isTowerCostMoreThanCurrentGold()) {
			g.setColor(Color.red);
			g.drawString("Не вистачає", 276, 725);
		}
	}
	...
	private void drawGoldAmount(Graphics g) {
		g.drawString("Золото: " + gold, 110, 725);
	...
	private void drawWaveLeftInfo(Graphics g) {
		...
		g.drawString("Хвиля " + (current + 1) + " / " + size, 425, 770);

	}

	private void drawEnemiesLeftInfo(Graphics g) {
		int remaining = playing.getEnemyManager().getAmountOfAliveEnemies();
		g.drawString("Ворогів: " + remaining, 425, 790);
	}
	...
	private void drawWaveTimerInfo(Graphics g) {
		if (playing.getWaveManager().isWaveTimerStarted()) {
			...
			g.drawString("Наступна хвиля:" + formattedText, 425, 750);
		}
	...
	private void drawDisplayedTower(Graphics g) {
		if (displayedTower != null) {
			g.setColor(new Color(107, 133, 133));
			...
			g.drawString("" + Towers.GetName(displayedTower.getTowerType()), 475, 660);
			g.drawString("ID: " + displayedTower.getId(), 475, 675);
			g.drawString("| LVL:" + displayedTower.getTier(), 560, 660);
			...
			// up b
			if(displayedTower.getTier() <3 && gold >=getSellAmount(displayedTower)) {
				upgradeTower.draw(g);
			...
			if(sellTower.isMouseOver()) {
				...
				g.drawString("Продати за: " + getSellAmount(displayedTower) + "з" , 475, 695);
				
			}else if(upgradeTower.isMouseOver() && gold >=getSellAmount(displayedTower) ) {
				g.setColor(Color.green);
				g.drawString("Покращити за: " + getUpgradeAmount(displayedTower) + "з" , 475, 695);
			}
		...
		if (bMenu.getBounds().contains(x, y))
			...
			if(displayedTower != null)
				if (sellTower.getBounds().contains(x, y)) {
					sellTowerClicked();
					return;
				} else if (upgradeTower.getBounds().contains(x, y) && displayedTower.getTier() <3 && gold >=getSellAmount(displayedTower) ) {
					upgradeTowerClicked();
					return;
				}
			...
			for (MyButton b : towerButtons) {
				if (b.getBounds().contains(x, y)) {
					...
					playing.setSeceltedTower(selectedTower);
					return;
