...
public class Utilz {
	...
	public static int[][] GetRoadDirArr(int[][] lvlTypeArr, RoadPoint start, RoadPoint end) {
		int[][] roadDirArr = new int[lvlTypeArr.length][lvlTypeArr[0].length];

		RoadPoint currTile = start;
		int lastDir = -1;

		while (!IsCurrSameAsEnd(currTile, end)) {
			RoadPoint prevTile = currTile;
			currTile = GetNextRoadTile(prevTile, lastDir, lvlTypeArr);
			lastDir = GetDirFromPrevToCurrr(prevTile, currTile);
	...
	private static int GetDirFromPrevToCurrr(RoadPoint prevTile, RoadPoint currTile) {
		// Up or down
	...
	private static RoadPoint GetNextRoadTile(RoadPoint prevTile, int lastDir, int[][] lvlTypeArr) {

		int testDir = lastDir;
		RoadPoint testTile = GetTileInDir(prevTile, testDir, lastDir);
	...
	private static boolean IsTileRoad(RoadPoint testTile, int[][] lvlTypeArr) {
		if (testTile != null)
	...
	private static RoadPoint GetTileInDir(RoadPoint prevTile, int testDir, int lastDir) {

		switch (testDir) {
		case LEFT:
			if (lastDir != RIGHT)
				return new RoadPoint(prevTile.getxCord() - 1, prevTile.getyCord());
		case UP:
			if (lastDir != DOWN)
				return new RoadPoint(prevTile.getxCord(), prevTile.getyCord() - 1);
		case RIGHT:
			if (lastDir != LEFT)
				return new RoadPoint(prevTile.getxCord() + 1, prevTile.getyCord());
		case DOWN:
	...
	private static boolean IsCurrSameAsEnd(RoadPoint currTile, RoadPoint end) {
	...
	public static int GetHypeDistance(float x1, float y1, float x2, float y2) {
